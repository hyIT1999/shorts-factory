...
- Vị trí: `DATA_DIR/music-library/` (hằng `MUSIC_LIBRARY_DIR`, mặc định `music-library`, phải nằm trong DATA_DIR để `storage.resolve()` dùng được). **Không** đưa vào `DATA_KINDS` nên `listDataDirs`, `projectDirs`, `data:sweep` bỏ qua (thư viện không thuộc project nào). VOICE `cleanupStaleAudio` (`lib/voice/service.ts`) chỉ xoá Asset `type:'audio'` / `type:'narration'` và file trong `audio/<p>/<v>/` nên không đụng tới nhạc; ASSETS chỉ xoá `VISUAL_ASSET_TYPES`. Lưu ý: từ 2026-09-22 thư mục `audio/<p>/<v>/` còn chứa `narration.wav` (cả lời đọc, VOICE_MODE=narration); MUSIC vẫn chỉ đọc `scene-NN.wav` như thiết kế và phải bỏ qua file này.
